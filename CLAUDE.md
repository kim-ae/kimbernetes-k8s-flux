...
Cilium runs **`1.18.13`** (upgraded from `1.18.0` on 2026/08/19 after a crash-loop root-caused to a known upstream Hubble segfault, [cilium/cilium#40804](https://github.com/cilium/cilium/issues/40804), fixed upstream since `1.18.1`; full incident in `diary/DIARY.md` 2026/08/19). Cilium's own Helm release is **not** managed via this repo — only the `CiliumL2AnnouncementPolicy` CR lives under `overlays/kimawesome/loadbalancer/`; the release itself is installed/upgraded imperatively (`helm`/`cilium-cli` directly against the cluster), so there is no Flux `HelmRelease` to check for drift and no GitOps reconciliation risk from an out-of-band upgrade. For any future Cilium version bump, use `cilium upgrade --version <target> --dry-run-helm-values` to preview, then `cilium upgrade --version <target> --wait` — **do not** use a raw `helm upgrade --reuse-values`, which reuses the *previous release's fully-resolved* `image.tag`/`image.digest`/`image.useDigest` (the old chart's own pinned defaults) and silently keeps the old container image running even though Helm reports the new chart version deployed; `cilium upgrade`'s default `--reset-then-reuse-values` resets to the new chart's own image defaults first, then reapplies custom values (`kubeProxyReplacement`, `l2announcements.enabled`, `routingMode`/`tunnelProtocol`, `k8sServiceHost`/`Port`, etc.) on top. Separately: both CoreDNS replicas (`coredns-*` in `kube-system`) are currently scheduled on the same node, `controlplane-82md` — a real single point of failure, not yet fixed, worth addressing with pod anti-affinity.
